...
public class GammaGraphUtils {
	...
	public static void drawGraphBase(Canvas canvas, GammaGraphInfo gammaGraphInfo, List<RectF> boundLines) {
		...
		canvas.drawText("0", baseRect.left - textPadding, baseRect.bottom + textPadding, paint);
		canvas.drawText("255", baseRect.left - textPadding - 10, baseRect.top, paint);
		canvas.drawText("(X)", baseRect.left - textPadding - 7, baseRect.top + textPadding, paint);
		canvas.drawText("255", baseRect.right, baseRect.bottom + textPadding, paint);
		canvas.drawText("(Y)", baseRect.right + 3, baseRect.bottom + textPadding * 2, paint);

		paint = new Paint();
		paint.setAntiAlias(true);
		paint.setColor(Color.GRAY);
		paint.setAlpha(70);
		canvas.drawLine(baseRect.left, baseRect.bottom, baseRect.right, baseRect.top, paint);
		if (boundLines != null) {
			for (RectF boundLine : boundLines) {
				canvas.drawLine(boundLine.left, boundLine.top, boundLine.right, boundLine.bottom, paint);
			}
		}
	}
	...
	public static void drawGammaHistogram(Canvas canvas, GammaGraphInfo gammaGraphInfo) {
		...
		for (int i = 0; i < histogramData.length; i++) {
			canvas.drawLine(baseRect.left + i * zoom, baseRect.bottom, baseRect.left + i * zoom, baseRect.bottom - histogramData[i] * zoom + 2, paint);
		}
	}

	public static void drawGammaGraph(final Canvas canvas, List<PointF> knots, int color, int alpha) {
		Log.d("GammaGraph", "function drawGammaGraph");
		final Paint paint = new Paint();
		paint.setColor(color);
		paint.setAntiAlias(true);
		paint.setAlpha(alpha);
		doCubicHermiteSpline(knots, 0.0005f, (x, y) -> canvas.drawPoint(x, y, paint));
	}
	public static void generateGammaTable(final RectF baseRect, final List<PointF> knots, final int[] gammatable) {
		doCubicHermiteSpline(knots, 0.0005f, (x, y) -> {
	...
	public static RectF createBaseRect(RectF graphRect, RectF marginRect) {
		...
		return new RectF(graphRect.left + marginRect.left, graphRect.top + marginRect.top, graphRect.right, graphRect.bottom);
	}
	public static List<RectF> createBoundLines(RectF baseRect, final float graphSideZoom) {
		...
		List<RectF> boundLines = new ArrayList<>();
		final float drawRectSide = baseRect.right - baseRect.left;
		final float rectSideQuadDiv = drawRectSide / 4;
		for (int i = 1; i <= 3; i++) {
			final float linePos = i * rectSideQuadDiv;
			RectF boundLine1 = new RectF(baseRect.left + linePos, baseRect.top + 1, baseRect.left + linePos, baseRect.bottom - 1);
			RectF boundLine2 = new RectF(baseRect.left + 1, baseRect.top + linePos, baseRect.right - 1, baseRect.top + linePos);
